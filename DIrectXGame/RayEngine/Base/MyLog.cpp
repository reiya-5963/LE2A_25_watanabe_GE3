...
std::wstring MyLog::ConvertString(const std::string& str) {
...
std::string MyLog::ConvertString(const std::wstring& str) {
	if (str.empty()) {
		return std::string();
	}
	...
	WideCharToMultiByte(CP_UTF8, 0, str.data(), static_cast<int>(str.size()),
		LPSTR(result.data()), sizeNeeded, NULL, NULL);
	return result;
}

void MyLog::Log(const std::string& message) {
